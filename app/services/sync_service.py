"""
Synchronisierungs-Service für Turnierdaten - VERSION 2.3
TEIL 1: API-Funktionen und Helper

✅ FIX: Entry.discipline_ids wird gespeichert
✅ FIX: Group-Validierung bei Matches
"""
import os
import requests
import json
...
def fetch_tournament_data(
    ...
    try:
        sync_logger.info(f"🌐 API-Call: GET {API_BASE}/{t_id}")
...
def fetch_tournament_entries(t_id: str, discipline_id: Optional[str] = None) -> Tuple[bool, Optional[List], Optional[str]]:
    ...
    try:
        ...
        sync_logger.info(f"📊 Entries API: {len(entries)} entries")
...
"""
TEIL 2: Haupt-Synchronisations-Logik
"""

def sync_tournament_data(t_id: str, data: Dict[str, Any]) -> Tuple[bool, str]:
    """
    Synchronisiert ALLE Turnierdaten in DB - VERSION 2.3
    
    ✅ BUGFIX: Entry.discipline_ids wird gespeichert
    ...
    try:
        ...
        # STEP 2: COURTS
        sync_logger.info("\n🔹 STEP 2: Courts")
        courts_saved = _sync_courts(t_id, data)
        
        # STEP 3: DISCIPLINES → STAGES → GROUPS
        sync_logger.info("\n🔹 STEP 3: Disciplines, Stages, Groups")
        disciplines_saved, stages_saved, groups_saved = _sync_structure(t_id, data)
        
        # STEP 4: ENTRIES mit Discipline-Zuordnung
        sync_logger.info("\n🔹 STEP 4: Entries mit Discipline-Zuordnung")
        entries_saved = _sync_entries(t_id, data.get('disciplines', []))
        
        # STEP 5: STANDINGS
        sync_logger.info("\n🔹 STEP 5: Standings")
        total_standings = _sync_standings(t_id, data.get('disciplines', []))
        
        # STEP 6: MATCHES (nur bei laufenden Turnieren)
        sync_logger.info("\n🔹 STEP 6: Matches")
        matches_saved = _sync_matches(t_id, tournament.state, data.get('disciplines', []))
        ...
        # VERIFICATION
        _verify_sync(t_id, entries_saved)
...
def _sync_courts(t_id: str, data: Dict[str, Any]) -> int:
    """
    Synchronisiert Courts
    ✅ BUGFIX: Lädt Courts IMMER über separate API, nicht nur aus Tournament-Response
    """
    courts_saved = 0
    
    # Versuche zuerst aus Tournament-Response
    courts_data = data.get('courts', [])
    
    # ✅ FIX: Wenn keine Courts in Response, lade separat über API
    if not courts_data:
        sync_logger.info(f"⚠️ Keine Courts in Tournament-Response - lade separat über API...")
        
        success, courts_data, error = fetch_courts(t_id, include_match_details=True)
        
        if not success:
            sync_logger.warning(f"⚠️ Konnte Courts nicht laden: {error}")
            return 0
        
        if not courts_data:
            sync_logger.info(f"ℹ️ Tournament hat keine Courts konfiguriert")
            return 0
    
    # Courts verarbeiten
    sync_logger.info(f"📥 {len(courts_data)} Courts gefunden")
    
    for c in courts_data:
        if not c.get('id'):
            sync_logger.warning(f"⚠️ Court ohne ID übersprungen")
            continue
        ...
        sync_logger.debug(f"  💾 Court {c.get('number')}: {c.get('name')}")
...
def _sync_structure(t_id: str, data: Dict[str, Any]) -> Tuple[int, int, int]:
    """Synchronisiert Disciplines, Stages, Groups"""
    ...
    return disciplines_saved, stages_saved, groups_saved


def _sync_entries(t_id: str, disciplines: List[Dict[str, Any]]) -> int:
    """
    Synchronisiert Entries mit Discipline-Zuordnung
    ✅ BUGFIX: discipline_ids wird jetzt gespeichert
    """
    all_entries = {}
    entries_per_discipline = {}
    
    # Lade Tournament-Entries (Basis)
    ...
    for d in disciplines:
        ...
        if disc_entries_success and disc_entries_data:
            sync_logger.info(f"  ✅ {len(disc_entries_data)} Entries")
    ...
    # ✅ BUGFIX: Speichere Entries MIT discipline_ids
    entries_saved = 0
    for entry_id, entry_data in all_entries.items():
        ...
        entry = Entry(
            ...
            discipline_ids=disciplines_list if disciplines_list else None  # ✅ FIXED!
        )
...
def _sync_standings(t_id: str, disciplines: List[Dict[str, Any]]) -> int:
    """Synchronisiert Standings"""
    total_standings = 0
...
def _sync_matches(t_id: str, tournament_state: str, disciplines: List[Dict[str, Any]]) -> int:
    """Synchronisiert Matches"""
    matches_saved = 0
    
    if tournament_state == 'finished':
        sync_logger.warning(
            "⚠️ Tournament ist 'finished' - API liefert keine Matches mehr. "
            "Matches müssen via Webhooks synchronisiert werden."
        )
        return 0
    
    # Verarbeite Matches aus API-Response
    for d in disciplines:
        stages = d.get('stages', [])
        for s in stages:
            groups = s.get('groups', [])
            for g in groups:
                ...
                if matches:
                    for m in matches:
                        ...
                        sync_match_to_db(t_id, m)
                        matches_saved += 1
    
    sync_logger.info(f"✅ Matches: {matches_saved} verarbeitet")
    
    return matches_saved


def _verify_sync(t_id: str, entries_saved: int):
    """Verifiziert Sync-Ergebnisse"""
    sync_logger.info("\n🔹 VERIFICATION:")
    
    verify_results = {
        "entries": Entry.query.filter_by(tournament_id=t_id).count(),
        "entries_with_disciplines": Entry.query.filter(
            Entry.tournament_id == t_id,
            Entry.discipline_ids.isnot(None)
        ).count(),
    ...
    sync_logger.info(f"  - Entries mit Disciplines: {verify_results['entries_with_disciplines']}")
    ...
    # Warnungen
    if verify_results['entries_with_disciplines'] == 0 and entries_saved > 0:
        sync_logger.error(f"❌ KEINE ENTRIES MIT DISCIPLINE-ZUORDNUNG!")

"""
TEIL 3: Match-Synchronisation

✅ BUGFIX: Group-Validierung hinzugefügt
"""

def sync_match_to_db(t_id: str, match_data: Dict[str, Any]):
    """
    Speichert Match in DB
    ✅ BUGFIX: Validiert Group-Existenz vor Speicherung
    """
    try:
        ...
        # ✅ BUGFIX: Prüfe ob Group existiert
        if group_id:
            group_exists = Group.query.get(group_id)
            if not group_exists:
                sync_logger.warning(
                    f"⚠️ Match {match_id} übersprungen: Group {group_id} fehlt in DB"
                )
                return
        else:
            sync_logger.warning(f"⚠️ Match {match_id} hat keine groupId")
            return
        ...
        if len(entries_data) >= 1 and entries_data[0]:
            if isinstance(entries_data[0], dict):
                ...
                team1_name = ' / '.join(names) if names else 'TBD'
        ...
        if len(entries_data) >= 2 and entries_data[1]:
            if isinstance(entries_data[1], dict):
                ...
                team2_name = ' / '.join(names) if names else 'TBD'
        ...
        sync_logger.debug(f"✅ Match {match_id}: {team1_name} vs {team2_name}")
